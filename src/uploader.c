...
static void create_bucket_entry(uv_work_t *work)
{
    ...
    int path_len = (uint32_t) strlen(state->bucket_id) + 16;
    ...
    if (!req->error_status &&
        ...
        if (shard->push_shard_request_count == 6) {
            ...
            if (state->exclude == NULL) {
                ...
                int new_len = (uint32_t) strlen(state->exclude) + (uint32_t) strlen(shard->pointer->farmer_node_id) + 1;
...
static void queue_verify_file_name(storj_upload_state_t *state)
{
    ...
    char *escaped = curl_easy_escape(curl, state->encrypted_file_name,
                                   (uint32_t)  strlen(state->encrypted_file_name));
...
char *create_tmp_name(storj_upload_state_t *state, char *extension)
{
    ...
    int file_name_len = (uint32_t) strlen(state->encrypted_file_name);
    int extension_len = (uint32_t) strlen(extension);
    int tmp_folder_len = (uint32_t) strlen(tmp_folder);

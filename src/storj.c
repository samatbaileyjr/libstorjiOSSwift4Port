...
#ifdef _POSIX_MEMLOCK
        int pass_len = (uint32_t) strlen(options->pass);
...
#ifdef _POSIX_MEMLOCK
        int mnemonic_len = (uint32_t) strlen(encrypt_options->mnemonic);
...
int storj_destroy_env(storj_env_t *env)
{
    ...
    if (env->bridge_options->pass) {
        unsigned int pass_len = (uint32_t) strlen(env->bridge_options->pass);
        if (pass_len > 0) {
    ...
    if (env->encrypt_options && env->encrypt_options->mnemonic) {
        unsigned int mnemonic_len = (uint32_t) strlen(env->encrypt_options->mnemonic);
...
{
    char *pass_encrypted;
    int pass_length = (uint32_t) strlen(bridge_pass);
    ...
    int mnemonic_length = (uint32_t) strlen(mnemonic);
                          ...
                          uv_after_work_cb cb)
{
    ...
    sha256_of_str((uint8_t *)password, (uint32_t)strlen(password), sha256_digest);

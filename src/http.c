              ...
              bool *canceled)
{
    ...
    int url_len = (uint32_t) strlen(proto) + 3 + (uint32_t) strlen(host) + 1 + 10 + 8
        + (uint32_t) strlen(shard_hash) + (uint32_t) strlen(query_args);
    ...
    *status_code = (uint32_t)_status_code;
                ...
                bool *canceled)
{
    ...
    char query_args[80];
    snprintf(query_args, 80, "?token=%s", token);
    int url_len = (uint32_t) strlen(proto) + 3 + (uint32_t) strlen(host) + 1 + 10
        + 8 + (uint32_t) strlen(shard_hash) + (uint32_t) strlen(query_args);
    char *url = calloc(url_len + 1, sizeof(char));
    if (!url) {
        return 1;
    }
               ...
               int *status_code)
{
    ...
    // Set the url
    int url_len = (uint32_t) strlen(options->proto) + 3 + (uint32_t) strlen(options->host) +
        1 + 10 + (uint32_t) strlen(path);
    char *url = calloc(url_len + 1, sizeof(char));
    if (!url) {
        return 1;
    }
    ...
    if (auth && options->user && options->pass) {
        ...
        int user_pass_len = (uint32_t) strlen(options->user) + 1 + (uint32_t) strlen(pass);
